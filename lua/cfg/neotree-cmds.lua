...
commands.zo = function(state, open_all)
...
commands.zO = function(state)
    commands.zo(state, true)
end
...
commands.zc = function(state, close_all)
    local node = state.tree:get_node()
    if not node then
        return
    end
...
commands.zC = function(state)
    commands.zc(state, true)
end

--- Open a closed folder or close an open one, with an optional count.
commands.za = function(state, toggle_all)
    local node = state.tree:get_node()
    if not node then
        return
    end
    ...
    if node.type == "directory" and not node:is_expanded() then
        commands.zo(state, toggle_all)
    else
        commands.zc(state, toggle_all)
    end
end

--- Recursively close an open folder or recursively open a closed folder.
commands.zA = function(state)
    commands.za(state, true)
end
